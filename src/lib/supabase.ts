...
export async function deleteDeposit(depositId: string): Promise<boolean> {
  try {
    ...
    const { data, error } = await supabase.rpc('delete_deposit_with_session', {
      deposit_id_input: depositId,
...
export async function deleteWithdrawal(withdrawalId: string): Promise<boolean> {
  try {
    ...
    const { data, error } = await supabase.rpc('delete_withdrawal_with_session', {
