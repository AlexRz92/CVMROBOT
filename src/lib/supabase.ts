...
export async function deleteDeposit(depositId: string): Promise<boolean> {
  try {
    console.log('Intentando eliminar depósito:', depositId);
    ...
    if (selectError) {
      console.error('Error al buscar depósito:', selectError);
      return false;
    }
    ...
    console.log('Depósito encontrado:', beforeDelete);

    if (beforeDelete.status !== 'pending') {
      console.error('El depósito no está en estado pendiente:', beforeDelete.status);
      return false;
    }
    ...
    console.log('Depósito eliminado exitosamente:', data);
    return true;
  } catch (error) {
    console.error('Excepción al eliminar depósito:', error);
    return false;
  }
}
...
export async function deleteWithdrawal(withdrawalId: string): Promise<boolean> {
  try {
    console.log('Intentando eliminar retiro:', withdrawalId);
    ...
    if (selectError) {
      console.error('Error al buscar retiro:', selectError);
      return false;
    }

    if (!beforeDelete) {
      console.error('Retiro no encontrado:', withdrawalId);
      return false;
    }

    console.log('Retiro encontrado:', beforeDelete);

    if (beforeDelete.status !== 'pending') {
      console.error('El retiro no está en estado pendiente:', beforeDelete.status);
      return false;
    }
    ...
    console.log('Retiro eliminado exitosamente:', data);
    return true;
  } catch (error) {
    console.error('Excepción al eliminar retiro:', error);
    return false;
  }
}
