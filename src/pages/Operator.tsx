...
export default function Operator() {
  ...
  const handleRejectUserSubmit = async () => {
    if (!user || !selectedPendingUser || !rejectionReason.trim()) return;

    const success = await rejectUser(selectedPendingUser.id, user.id, rejectionReason);
  ...
  const openUserRejectModal = (user: PendingUser) => {
          ...
          >
            ...
            {subView === 'pending' && (
              <div className="flex gap-2 pt-3">
                ...
                <button
                  onClick={() => openUserRejectModal(userItem)}
                  className="flex-1 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
                >
                  <XCircle className="w-4 h-4" />
                  Rechazar
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex">
      ...
      <div className="relative z-10 w-64 bg-slate-900/50 backdrop-blur-xl border-r border-cyan-500/20 p-6 flex flex-col">
        ...
        <nav className="flex-1 space-y-2">
          <button
            onClick={() => { setMainView('deposits'); setSubView('pending'); }}
          ...
          <button
            onClick={() => { setMainView('withdrawals'); setSubView('pending'); }}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-all ${
          ...
          <button
            onClick={() => setMainView('bot-activation')}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-all ${
          ...
          <button
            onClick={() => { setMainView('user-approval'); setSubView('pending'); }}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-all ${
      ...
      {showUserRejectModal && selectedPendingUser && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-red-500/30 rounded-2xl max-w-md w-full p-6">
            <h3 className="text-2xl font-bold text-red-400 mb-4">Rechazar Usuario</h3>
            <p className="text-gray-300 mb-2">
              Usuario: <span className="font-bold text-white">{selectedPendingUser.nombre} {selectedPendingUser.apellido}</span>
            </p>
