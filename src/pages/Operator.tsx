...
import { CheckCircle, XCircle, LogOut, Power, Users, UserCheck, Wallet, Sparkles, Plus, Edit, Trash2 } from 'lucide-react';
...
type MainView = 'bot-activation' | 'user-approval' | 'users' | 'plans';
      ...
      <div className="relative z-10 flex-1 p-8 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl font-bold text-white mb-6">
            {mainView === 'bot-activation' ? 'Activación del Bot' : mainView === 'user-approval' ? 'Aprobación de Usuarios' : mainView === 'users' ? 'Usuarios por Exchange' : 'Gestion de Planes'}
          </h2>
          ...
          <div className={`${mainView === 'users' || mainView === 'plans' ? '' : 'bg-slate-800/50 backdrop-blur-xl border border-cyan-500/20 rounded-2xl p-8 shadow-[0_0_50px_rgba(34,211,238,0.15)]'}`}>
            {loading ? (
              <div className="text-center py-12 text-cyan-400">Cargando...</div>
            ) : (
              mainView === 'bot-activation' ? renderBotActivation() : mainView === 'user-approval' ? renderUserApproval() : mainView === 'users' ? renderUsersByExchange() : renderPlans()
            )}
          </div>
        </div>
      </div>
