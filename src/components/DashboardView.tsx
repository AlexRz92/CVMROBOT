...
import { DollarSign, Send, Wallet, LogOut as LogOutIcon, MessageSquare, Clock, Sparkles, CheckCircle, Info, X } from 'lucide-react';
...
export default function DashboardView() {
  ...
  useEffect(() => {
    const loadPlans = async () => {
      const [activePlans, plansEnabledConfig] = await Promise.all([
        getActivePlans(),
  ...
  const handleCurrentPlanClick = () => {
    if (userPlan && userPlan.plan) {
      setSelectedPlanForDetails(userPlan.plan);
      setShowCurrentPlanModal(true);
    }
  };

  const handleConfirmPlanSelection = async () => {
    if (!user || !selectedPlanForDetails) return;

    setSubmitting(true);
    setError('');

    try {
      const result = await createPlanChangeRequest(user.id, selectedPlanForDetails.id);
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="space-y-4">
              ...
              {userPlan && userPlan.plan && (
                <div
                  ...
                  className="bg-gradient-to-br from-cyan-900/40 to-blue-900/40 border border-cyan-700/50 rounded-lg p-4 cursor-pointer hover:border-cyan-500/70 transition-all"
                >
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs text-gray-400">Plan Activo</p>
                  ...
                  <p className="text-xs text-cyan-500 mt-2 font-medium">Click para ver detalles</p>
                </div>
              )}
            </div>
          ) : (
        ...
        <div className="lg:col-span-2 bg-gradient-to-br from-cyan-900/30 to-blue-900/30 backdrop-blur-sm border border-cyan-700/50 rounded-xl p-6 relative">
          <div className="flex items-center justify-between mb-4">
              ...
              {plans.map((plan) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-gradient-to-br from-cyan-900/40 to-blue-900/40 border rounded-lg p-4 transition-all ${
                      plansEnabled ? 'cursor-pointer' : 'cursor-not-allowed'
                    } ${
                      isCurrentPlan
          ...
          {!plansEnabled && (
            <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm rounded-xl flex items-center justify-center z-10">
              <div className="text-center p-6">
                ...
                <h4 className="text-2xl font-bold text-yellow-400 mb-2">En Construcción</h4>
                <p className="text-gray-300 text-sm">Esta sección está temporalmente deshabilitada</p>
              </div>
            </div>
          )}
        </div>
      ...
      {showPlanDetailsModal && selectedPlanForDetails && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-cyan-500/30 rounded-2xl max-w-xl w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-2xl font-bold text-cyan-400">Detalles del Plan</h3>
              <button
                onClick={() => {
                  setShowPlanDetailsModal(false);
                  setSelectedPlanForDetails(null);
            ...
            <div className="bg-gradient-to-br from-cyan-900/40 to-blue-900/40 border border-cyan-700/50 rounded-xl p-6 mb-6">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-2xl font-bold text-white">{selectedPlanForDetails.name}</h4>
                <span className="text-3xl font-bold text-cyan-400">${selectedPlanForDetails.price}</span>
              </div>
              <p className="text-gray-300 mb-4">{selectedPlanForDetails.description}</p>
              <div className="flex items-center gap-2 text-cyan-400">
                <Clock className="w-5 h-5" />
                <span className="font-medium">{selectedPlanForDetails.duration_days} días de acceso</span>
              </div>
            </div>

            {selectedPlanForDetails.features && selectedPlanForDetails.features.length > 0 && (
              <div className="mb-6">
                <h5 className="text-lg font-bold text-white mb-3">Características:</h5>
                <ul className="space-y-2">
                  {selectedPlanForDetails.features.map((feature, idx) => (
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 mb-6">
              ...
              <p className="text-gray-300 text-sm mt-2">
                Los días restantes de tu plan actual no se acumularán con este plan. Al cambiar, comenzarás con {selectedPlanForDetails.duration_days} días desde cero.
              </p>
            </div>
            ...
            <div className="flex gap-4">
              <button
                ...
                disabled={submitting}
                className="flex-1 py-3 bg-slate-700 text-white rounded-lg hover:bg-slate-600 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={submitting}
                className="flex-1 py-3 bg-gradient-to-r from-cyan-600 to-blue-600 text-white rounded-lg hover:shadow-lg transition-all font-medium disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {submitting ? 'Procesando...' : 'Seleccionar Plan'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showCurrentPlanModal && selectedPlanForDetails && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-emerald-500/30 rounded-2xl max-w-xl w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-2xl font-bold text-emerald-400">Tu Plan Actual</h3>
              <button
                onClick={() => {
            ...
            <div className="bg-gradient-to-br from-emerald-900/40 to-green-900/40 border border-emerald-700/50 rounded-xl p-6 mb-6">
              <div className="flex items-center gap-3 mb-4">
                <CheckCircle className="w-8 h-8 text-emerald-400" />
                <div className="flex-1">
                  <h4 className="text-2xl font-bold text-white">{selectedPlanForDetails.name}</h4>
                  <p className="text-emerald-400 font-medium">Plan Activo</p>
                </div>
                <span className="text-3xl font-bold text-emerald-400">${selectedPlanForDetails.price}</span>
              </div>
              <p className="text-gray-300 mb-4">{selectedPlanForDetails.description}</p>
            ...
            {selectedPlanForDetails.features && selectedPlanForDetails.features.length > 0 && (
              <div className="mb-6">
                <h5 className="text-lg font-bold text-white mb-3">Características de tu plan:</h5>
                <ul className="space-y-2">
                  {selectedPlanForDetails.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-gray-300">
            ...
            {userPlan && (
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 mb-6">
                <p className="text-blue-400 font-medium text-sm mb-2">Información de tu suscripción:</p>
                <div className="space-y-1 text-sm text-gray-300">
                  <p>Activado: {new Date(userPlan.activated_at).toLocaleDateString('es-ES')}</p>
                  {userPlan.expires_at && (
                    <p>Expira: {new Date(userPlan.expires_at).toLocaleDateString('es-ES')}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <button
              ...
              className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors font-medium"
            >
              Cerrar
            </button>
          </div>
