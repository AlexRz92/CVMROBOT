...
import { TrendingUp, DollarSign, Calendar, Download, Upload, X, Send, Trash2 } from 'lucide-react';
...
export default function DashboardView() {
  ...
  const [selectedRejection, setSelectedRejection] = useState<{ type: string; reason: string } | null>(null);
  const [selectedTransaction, setSelectedTransaction] = useState<{ id: string; type: 'deposit' | 'withdrawal' } | null>(null);
  ...
  const handleDeleteTransaction = async () => {
    if (!selectedTransaction || !user || deletingTransaction) return;

    console.log('Iniciando eliminación de transacción:', selectedTransaction);
    setDeletingTransaction(true);
    setDeleteError('');

    try {
      const success = selectedTransaction.type === 'deposit'
        ? await deleteDeposit(selectedTransaction.id)
        : await deleteWithdrawal(selectedTransaction.id);
      ...
      if (success) {
        ...
        const [userDeposits, userWithdrawals, calculatedBalance] = await Promise.all([
          getUserDeposits(user.id),
          getUserWithdrawals(user.id),
          getCalculatedBalance(user.id),
        ]);
        ...
        console.error('No se pudo eliminar la transacción');
        setDeleteError('No se pudo eliminar la transacción. Por favor intenta de nuevo.');
      }
    } catch (err) {
      console.error('Error al eliminar transacción:', err);
      setDeleteError('Error al eliminar la transacción. Por favor intenta de nuevo.');
    } finally {
    ...
    <div className="space-y-6 relative">
        ...
        <div className="bg-blue-900/30 backdrop-blur-sm border border-blue-700/50 rounded-xl p-6">
          ...
          <div className="space-y-4">
            {[
              ...
              const exchangeBalances = getExchangeBalances();
              const balance = exchangeBalances[exchange.key];
              const isConnected = balance > 0;
      ...
      <div className="bg-blue-900/30 backdrop-blur-sm border border-blue-700/50 rounded-xl p-6">
        ...
        <div className="overflow-x-auto max-h-64 overflow-y-auto">
          <table className="w-full">
            <thead className="sticky top-0 bg-blue-900/50 backdrop-blur-sm">
                    ...
                    <td className="py-3 px-4 text-center">
                      <span
                        className={`inline-flex px-3 py-1 rounded-full text-xs font-medium ${
                          ...
                          transaction.status === 'rejected' ? 'bg-red-500/20 text-red-400 cursor-pointer hover:bg-red-500/30' :
                          'bg-yellow-500/20 text-yellow-400 cursor-pointer hover:bg-yellow-500/30'
                        }`}
                        onClick={() => {
                          if (transaction.status === 'rejected' && transaction.rejection_reason) {
                            setSelectedRejection({
                              type: isDeposit ? 'Depósito' : 'Retiro',
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-emerald-500/30 rounded-2xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleDeposit} className="space-y-4">
              ...
              >
                {submittingDeposit ? 'Procesando...' : 'Solicitar Depósito'}
              </button>
            </form>
          </div>
        </div>
            ...
            {getExchangesWithBalance().length === 0 ? (
              ...
              <form onSubmit={handleWithdrawal} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3 bg-gradient-to-r from-red-600 to-rose-600 text-white font-semibold rounded-lg hover:shadow-[0_0_30px_rgba(220,38,38,0.5)] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      {showRejectionModal && selectedRejection && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-red-500/30 rounded-2xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-2xl font-bold text-red-400">{selectedRejection.type} Rechazado</h3>
              <button
                onClick={() => {
            ...
            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
              ...
              <p className="text-white">{selectedRejection.reason}</p>
            </div>
      ...
      {showDeleteModal && selectedTransaction && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-yellow-500/30 rounded-2xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-2xl font-bold text-yellow-400">Eliminar Transacción</h3>
              <button
                onClick={() => {
            ...
            <div className="flex gap-4">
              ...
              >
                {deletingTransaction ? 'Eliminando...' : 'Sí, eliminar'}
              </button>
            </div>
          </div>
        </div>
