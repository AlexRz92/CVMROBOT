...
import { DollarSign, Send, Wallet, LogOut as LogOutIcon, MessageSquare, Clock } from 'lucide-react';
...
export default function DashboardView() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      const [botAct, userCapital, calculatedBalance] = await Promise.all([
        ...
        <div className="lg:col-span-2 bg-blue-900/30 backdrop-blur-sm border border-blue-700/50 rounded-xl p-6">
          ...
          {capital ? (
            <div className="space-y-4">
              <div className="bg-gradient-to-br from-emerald-900/40 to-green-900/40 border border-emerald-700/50 rounded-lg p-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 rounded-full bg-emerald-500/20 flex items-center justify-center">
                    <Wallet className="w-6 h-6 text-emerald-400" />
                  </div>
                  <div>
                    <p className="font-bold text-white text-lg">Capital Invertido</p>
                    <p className="text-xs text-gray-400 capitalize">{capital.exchange}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-3xl font-bold text-emerald-400">${capital.capital_amount.toFixed(2)}</p>
                </div>
              </div>

              <button
                onClick={() => setShowWithdrawModal(true)}
                className="w-full group relative overflow-hidden rounded-xl p-6 bg-gradient-to-br from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 transition-all transform hover:scale-105 shadow-lg hover:shadow-xl"
              >
                ...
                <div className="relative flex items-center justify-center gap-3">
                  ...
                  <div>
                    <p className="text-base font-bold text-white">Retiro Completo</p>
            ...
            <div className="space-y-4">
              <div className="text-center py-8 text-gray-400">
                ...
                <p className="mb-2">No tienes capital invertido</p>
                <p className="text-sm">Invierte en un exchange para comenzar</p>
              </div>

              <button
                onClick={() => setShowInvestModal(true)}
                className="w-full group relative overflow-hidden rounded-xl p-6 bg-gradient-to-br from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 transition-all transform hover:scale-105 shadow-lg hover:shadow-xl"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 group-hover:translate-x-full transition-transform duration-500"></div>
                <div className="relative flex items-center justify-center gap-3">
                  <DollarSign className="w-6 h-6 text-white" />
                  <div>
                    <p className="text-base font-bold text-white">Invertir Capital</p>
